...
def concat_abs_time(time_seq, item_idx, time_span, maxlen):
    ...
    time_abs_matrix = np.zeros([size, size], dtype=np.int32)
    for i in range(size):
        for j in range(size):
         time_abs_matrix[i][j] = min(time_seq[i], time_span * (maxlen // 10))
...
def Relation_abs(user_train, usernum, maxlen, time_span):
    ...
    data_train = dict()
    for user in tqdm(range(1, usernum+1), desc='Preparing relation matrix'):
        time_seq = np.zeros([maxlen, maxlen], dtype=np.int32)
        idx = maxlen - 1
        for i in reversed(user_train[user][:-1]):
            for j in range(maxlen):
                time_seq[idx][j] = min(i[1], time_span * (maxlen // 10))
            idx -= 1
            if idx == -1: break
...
def sample_function(user_train, usernum, itemnum, batch_size, maxlen, relation_matrix, relation_abs_matrix, result_queue, SEED):
    ...
    def sample(user):
        ...
        return (user, seq, time_seq, time_matrix, time_abs_matrix, pos, neg)
...
def evaluate(model, dataset, args):
    ...
    for u in users:
        ...
        time_abs_matrix = concat_abs_time(time_seq, item_idx, args.time_span, args.maxlen)
...
def new_evaluate(model, dataset, args):
    [train, valid, test, usernum, itemnum, timenum] = copy.deepcopy(dataset)
    ...
    for u in users:
        ...
        old_predictions = -old_predictions[0]  # 当new_predict()使用时 注掉
...
def new_evaluate_valid(model, dataset, args):
    [train, valid, test, usernum, itemnum, timenum] = copy.deepcopy(dataset)
